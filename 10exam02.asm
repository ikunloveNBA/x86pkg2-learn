...
start:
      ...
      or al,0x80                         ;阻断NMI 
      out 0x70,al
      mov al,01000010b                   ;允许周期更新，并且只允许周期性中断
      out 0x71,al

      mov al,0x0a                        ;选择寄存器a
      or al,0x80                         ;阻断NMI
      out 0x70,al                        ;写入索引寄存器
      in al,0x71                         ;读取寄存器a中的内容
      and al,11110000b                   ;将低四位置0
      ...
      mov al,0xfe                        ;清除bit 0(此位连接RTC)
      out 0xa1,al                        ;写回此寄存器

      mov al,11111011b                   ;只允许主片上的从片发生中断
      out 0x21,al                        ;写回主片IMR寄存器
